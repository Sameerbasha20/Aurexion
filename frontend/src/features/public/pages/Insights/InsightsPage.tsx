    ...
    if (isUsingDb) {
      return dbPosts.map(post => {
        ...
        if (!coverImage) {
          ...
          coverImage = (categoryImages as Record<string, any>)[cat.toLowerCase()] || "/images/unsplash_1486406146926-c6.webp";
        }
  ...
  const filteredArticles = useMemo(() => {
    ...
    return articlesList.filter((post) => {
      ...
      if (searchQuery) {
        ...
        const matchTags = post.tags.some((tag: any) => tag.toLowerCase().includes(query));
