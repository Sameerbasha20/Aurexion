...
export const CaseStudiesPage = () => {
  ...
  const mappedDbCaseStudies = useMemo(() => {
    return (dbCaseStudies || []).map((dbCase: any) => {
      const staticCaseStudy = caseStudiesData.find((cs: any) => cs.slug === dbCase.slug);
      ...
      return {
        ...
        services: staticCaseStudy?.services || (dbCase.services ? dbCase.services.map((s: any) => s.slug) : [])
      };
    });
  }, [dbCaseStudies]);
  ...
  const filteredCaseStudies = useMemo(() => {
    return allCaseStudies.filter((cs: any) => {
  ...
  const gridCaseStudies = filteredCaseStudies.filter((cs: any) => {
