...
// Curated Unsplash cover images per industry slug
...
export const IndustryCaseStudies = ({ industry }) => {
  ...
  // Resolve each csId to a real case study object
  const resolvedStudies = industry.relatedCaseStudies
    .map((csId) =>
      caseStudiesData.find(
        (cs) =>
          cs.slug === csId ||
          cs.id === csId ||
  ...
  if (resolvedStudies.length === 0) return null;
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {resolvedStudies.map((cs) => {
            ...
            const outcome = cs.results?.[0]
              ...
              : industry.outcomes?.[0]
              ? `${industry.outcomes[0]} Achieved`
              : "Operational Excellence Delivered";

            const industryLabel = (cs.industry || industry.slug)
              .replace(/-/g, " ")
